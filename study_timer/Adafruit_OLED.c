...
*/
    ...
    //dummy variable to capture/store unused data
    unsigned long ulDummy;

    //enable DC pin to indicate writeCommand
    GPIOPinWrite(GPIOA3_BASE, 0x80, 0x00);  //turn off pin 61 / DC

    //enable SPICS
    MAP_SPICSEnable(GSPI_BASE);

    //enable OLEDCS to activate OLED for reading
    GPIOPinWrite(GPIOA0_BASE, 0x80, 0x00);  //turn off P62 / CS|OS output signal


    //send char c to the OLED through SPI
    MAP_SPIDataPut(GSPI_BASE,c);

    //whatever unwanted data is put into dummy variable
    MAP_SPIDataGet(GSPI_BASE,&ulDummy);

    //disable SPICS
    MAP_SPICSDisable(GSPI_BASE);

    //disable OLEDCS, so we no longer can read from it
    GPIOPinWrite(GPIOA0_BASE, 0x80, 0x80);  //turn on P62 / CS|OS output signal
...
*/
    ...
    //enable DC pin to indicate writeData, this is the only difference \
    in writeData and writeCommand since we are setting writeData with high and \
    writeCommand with low
    GPIOPinWrite(GPIOA3_BASE, 0x80, 0x80);  //turn off pin 61 / DC
